...
class Basic implements Lang
{
    ...
    function check($handler, $node, $chainPos)
    {
        ...
        if (isset($this->rules[$hid])) {
            ...
            if (isset($rule['last']) && isset($node->chain[$chainPos+1]))
                throw new Exception\Check("Handlers may not follow '$hid' in a chain", $node->line);

            if (isset($rule['notFirst']) && $chainPos == 0)
                throw new Exception\Check("Handler '$hid' may not be first", $node->line);
    ...
    {
        return $this->handlers[$handler->name]($val, $context);
    }
}
