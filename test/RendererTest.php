...
class RendererTest extends \PHPUnit_Framework_TestCase
{
    ...
    function testRenderVarHandlerNoKeyNoFilter()
    {
        $tpl = '{{h}}';
        $tree = $this->parser->parse($tpl);
        
        $renderer = new Renderer;
        $renderer->varHandlers['h'] = function() {
            return 'bingo';
        };
     ...
     */
    function testRenderVarHandlerStringableOutput($var, $expected)
    {
        $tpl = '{{h}}';
        $tree = $this->parser->parse($tpl);
        
        $renderer = new Renderer;
        $renderer->varHandlers['h'] = function() use ($var) {
            return $var;
        };
    ...
    function testRenderVarHandlerKeyNoFilter()
    {
        $tpl = '{{h key}}';
        $tree = $this->parser->parse($tpl);
        
        $renderer = new Renderer;
        $renderer->varHandlers['h'] = function($scope, $key) {
            return 'bingo '.$key;
        };
        $this->assertEquals('bingo key', $renderer->render($tpl));
    }

    function testRenderVarHandlerKeyFilter()
    {
        $tpl = '{{h key|f1}}';
        $renderer = new Renderer;
        $renderer->varHandlers['h'] = function($scope, $key) {
    ...
    function testRenderVarHandlerKeyMultipleFilters()
    {
        $tpl = '{{h key|f1|f2}}';
        $renderer = new Renderer;
        $renderer->varHandlers['h'] = function($scope, $key) {
    ...
    function testRenderVarHandlerCallableFilter()
    {
        $tpl = '{{h key|f}}';
        $renderer = new Renderer;
        $renderer->varHandlers['h'] = function($scope, $key) {};
    ...
    function testRenderVarHandlerClassFilter()
    {
        $tpl = '{{h key|f.x}}';
        $renderer = new Renderer;
        $renderer->varHandlers['h'] = function($scope, $key) { return "value"; };
        
        $renderer->filters['f'] = new TestFilter;
        $this->assertEquals('x(value)', $renderer->render($tpl));
    }

    function testRenderVarHandlerClassFilterDoesntNest()
    {
        $tpl = '{{h key|f.x.x}}';
        $renderer = new Renderer;
        $renderer->varHandlers['h'] = function($scope, $key) { return "value"; };
    ...
    function testRenderBlockSubrender()
    {
        $tpl = '{{#h}}foo {{v}} bar{{/h}}';
        $renderer = new Renderer;
        $renderer->varHandlers['v'] = function($scope, $key) { return "hello"; };
    ...
    function testRenderBlockCanRenderTreeTwice()
    {
        ...
        $renderer = new Renderer;
        $renderer->varHandlers['v'] = function($scope, $key) { return "9"; };
        $renderer->blockHandlers['h'] = function($scope, $key, $contents) use ($renderer) {
            return "<"
    ...
    {
        $renderer = new Renderer;
        $renderer->varHandlers['do'] = function($key) { return 'handled'; };
